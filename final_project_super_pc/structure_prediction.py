...
class ProteinStructurePredictor6(keras.Model):
    def __init__(self):
        super().__init__()
        self.layer0 = keras.layers.Conv2D(7, 3, activation='gelu', padding="same")
        # self.layer1 = keras.layers.Conv2D(1, 1, activation='gelu', padding="same")
        self.attention = keras.layers.MultiHeadAttention(num_heads=4, key_dim=16)
        self.dense1 = keras.layers.Dense(64, activation='gelu')
        # self.dense2 = keras.layers.Dense(1, activation='gelu')
        self.resnet = res.resnet34()
    ...
    def call(self, inputs, mask=None):
        ...
        x = self.vit(x)
        # print("=============",x.shape)
        x = self.resnet(tf.expand_dims(x, axis=-1))
...
class ProteinStructurePredictor7(keras.Model):
    def __init__(self):
        ...
        self.attention = keras.layers.MultiHeadAttention(num_heads=4, key_dim=28)
        self.dense1 = keras.layers.Dense(64, activation='gelu')
        self.resnet = res.resnet34()
        self.add = keras.layers.Add()
    #@tf.function
...
class ProteinStructurePredictor8(keras.Model):
    def __init__(self):
        super().__init__()
        self.layer0 = keras.layers.Conv2D(13, 5, activation='gelu', padding="same")
        self.attention = keras.layers.MultiHeadAttention(num_heads=4, key_dim=42)
        self.dense1 = keras.layers.Dense(64, activation='gelu')
        self.dense2 = keras.layers.Dense(256, activation='gelu')
        # self.resnet = res.resnet34()
        self.res_u = res_u.resnet34()
        self.add = keras.layers.Add()
        self.conv1 = keras.layers.Conv2D(64,3,2,activation='gelu', padding='same')
        self.deconv1 = keras.layers.Conv2DTranspose(1,3,2,padding='same')
    ...
    def call(self, inputs, mask=None):
        ...
        x = self.add([tf.expand_dims(x, -2) , tf.expand_dims(x, -3)])
        # filter the initial representation into an embedded representation
        x = self.layer0(x * tf.expand_dims(mask, axis=-1))
        # add positional distance information
        r = tf.range(0, utils.NUM_RESIDUES, dtype=tf.float32)
        distances = tf.abs(tf.expand_dims(r, -1) - tf.expand_dims(r, -2))
        distances_bc = tf.expand_dims(
...
def main(data_folder):
    ...
    # with strategy.scope():
    model = ProteinStructurePredictor8()
    model.optimizer = keras.optimizers.Adam(learning_rate=1e-2)
    model.batch_size = 26 #1024
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0 and epoch > 0:
            new_lr = model.optimizer.learning_rate * 0.6
            model.optimizer.learning_rate.assign(new_lr)
            print(f"Epoch {epoch}: Learning rate is {new_lr.numpy()}")
