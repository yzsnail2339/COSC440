...
class BasicBlock(layers.Layer):
    ...
    def __init__(self, out_channel, strides=1, downsample=None, **kwargs):
        ...
        self.relu = layers.LeakyReLU(negative_slope = 0.3)
        self.add = layers.Add()
...
class BasicBlock_decoder(layers.Layer):
    ...
    def __init__(self, out_channel, strides=1, downsample=None, **kwargs):
        ...
        self.conv1 = layers.Conv2DTranspose(out_channel, kernel_size=3, strides=strides,
                                   padding="same", use_bias=False)
        self.bn1 = layers.BatchNormalization(momentum=0.9, epsilon=1e-5)
        # -----------------------------------------
        self.conv2 = layers.Conv2DTranspose(out_channel, kernel_size=3, strides=1,
                                   padding="same", use_bias=False)
        self.bn2 = layers.BatchNormalization(momentum=0.9, epsilon=1e-5)
        # -----------------------------------------
        self.downsample = downsample
...
class Decoder(tf.keras.layers.Layer):
    def __init__(self):
        ...
        self.decoder_activation4 = tf.keras.layers.LeakyReLU(negative_slope = 0.3)
        ...
        self.decoder_activation5 = tf.keras.layers.LeakyReLU(negative_slope = 0.3)
...
class attetion_operate(layers.Layer):
    def call(self, inputs):
        # print(f'attention_operate_shape is {x.shape}')
        d = inputs.shape[-1]
        x = tf.keras.layers.MultiHeadAttention(num_heads=1, key_dim=d)(inputs,inputs,inputs)
        x = tf.transpose(x,[0,2,1,3])
        x = tf.keras.layers.MultiHeadAttention(num_heads=1, key_dim=d)(x,x,x)
        x = tf.transpose(x,[0,2,1,3])
        return x
...
def _resnet(block, blocks_num, im_width=256, im_height=256):
    ...
    attention = attetion_operate()(x)
    x = ConcatenateLayer()([x, attention])
    x = _make_layer(BasicBlock_decoder, x.shape[-1], 256, blocks_num[3], strides=2, down=False, name="block5")(x)
    # attention = attetion_operate()(x)
    x = ConcatenateLayer()([x, x_block3])
    x = _make_layer(BasicBlock_decoder, x.shape[-1], 128, blocks_num[2], strides=2,down=False, name="block6")(x)
    x = ConcatenateLayer()([x, x_block2])
    x = _make_layer(BasicBlock_decoder, x.shape[-1], 64, blocks_num[1], strides=2,down=False, name="block7")(x)
