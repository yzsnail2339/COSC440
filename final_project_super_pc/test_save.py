...
class ProteinStructurePredictor5(keras.Model):
    ...
    #     # 确保父类的 build 方法被调用
    #     super(ProteinStructurePredictor5, self).build(input_shape)
    #     # 这里可以根据输入形状进行权重初始化，或定义层的输入维度
    #     # 示例：假设 input_shape 是 (batch_size, 255, 255, 3)
    #     # 可以在此初始化层的输入维度，或者根据需求动态调整模型结构
...
def main(data_folder):
    ...
    # model.build(input_shape=[(None,255, 21), (None,255, 255)])
    # model.summary()
