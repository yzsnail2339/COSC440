...
def pre_process_data(inputs, labels, first_class, second_class):
	...
	labels_site = np.where((labels == first_class) | (labels == second_class))
	inputs = np.reshape(inputs, (-1, 3, 32, 32))
	inputs = inputs[labels_site].astype(np.float32) / 255.0
	inputs = np.transpose(inputs, (0,2,3,1))
	labels = np.where(labels[labels_site] == first_class, 0, 1)
...
if __name__ == '__main__':
	...
	train_data_path = f'{cifar_data_folder}/train'
	test_data_path = f'{cifar_data_folder}/test'
	train_inputs, train_labels = get_data(train_data_path, 5, 3)
	test_inputs, test_labels = get_data(test_data_path, 5, 3)